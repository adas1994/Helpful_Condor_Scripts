...
if geom==35 and pm=='old':
    storage_base_dir = "/hadoop/store/user/adas/EfficiencyD41OldPreMixMethod/StandardMixOld/"+outdir_name
elif geom==35 and pm=='new':
    storage_base_dir = "/hadoop/store/user/adas/EfficiencyD41OldPreMixMethod/StandardMixNew/"+outdir_name
elif geom==41 and pm=='old':
    storage_base_dir = "/hadoop/store/user/adas/New_storeD41/StandardMixOld/"+outdir_name
elif geom==41 and pm=='new':
    storage_base_dir = "/hadoop/store/user/adas/New_storeD41/StandardMixNew/"+outdir_name
...
def submit_jobs(njobs, qsub_scriptname, dirname = None):
    ...
    for i in range(njobs):
        ...
        rep["file_listpath"]   = os.path.join(base_dir, "filesfor_StandardMixHGCTimingValidation.txt")
