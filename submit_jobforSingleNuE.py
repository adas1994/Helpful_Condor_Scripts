...
if geom==35 and pm=='old':
    storage_base_dir = "/hadoop/store/user/adas/New_storeD35/PreMixOld/SingleNuE"
elif geom==35 and pm=='new':
    storage_base_dir = "/hadoop/store/user/adas/New_storeD35/PreMixNew/SingleNuE"
elif geom==41 and pm=='old':
    storage_base_dir = "/hadoop/store/user/adas/New_storeD41/PreMixOld/SingleNuE"
elif geom==41 and pm=='new':
    storage_base_dir = "/hadoop/store/user/adas/New_storeD41/PreMixNew/SingleNuE"
...
tarfile_path = os.path.join(base_dir, CMSSW_BASE)+".tar.gz"
executable_name = "SingleNuE.csh"
...
def submit_jobs(njobs, qsub_scriptname, dirname = None):
    ...
    for i in range(njobs):
        ...
        if geom == 35:
            rep["configfile"]      = "SingleNuE10_cfi_GEN_SIM_DIGI_PU.py"
        elif geom == 41:
            rep["configfile"]      = "SingleNuE10_cfi_GEN_SIM_DIGI_PUD41.py"
        rep["tarfile_path"]    = tarfile_path
        rep["geom"]            = str(geom)
        rep["pm"]              = pm
