...
parser.add_argument("data_tag", help="data tag")
...
def modify_and_cmsRun_config_file(configfile, files_list, idx_primary):
    ...
    if geometry==41:
        gensim_file = gensim_file
    if geometry==35:
        raise NotImplementedError("Only Geometry D35 is being allowed for now !!!") 
    ...
    configfile_path = CMSSW_BASE+"/src/HGCTimingAnalysis/HGCTiming/test/"+configfile
    configfile_torun =  open(configfile_path, "r")
    substitute_file_path  = CMSSW_BASE+"/src/HGCTimingAnalysis/HGCTiming/test/substitute.py"
    ...
    os.system("./timing_ana.csh "+storage_dir+" "+str(idx_primary)+" "+data_tag)
