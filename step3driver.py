...
parser.add_argument("idx_Secondary", type=int, help="numeric index of the secondary file to be used")
...
idx_Secondary    = args.idx_Secondary
storage_dir      = args.storage_dir
geometry         = args.geometry
premix_method    = args.premix_method

def modify_and_cmsRun_config_file(configfile, files_list, idx_primary, idx_Secondary):
    ...
    assert (type(idx_Secondary) == int)
    idx_Secondary = idx_Secondary%250
    if geometry==35 and premix_method=='new':
        secondary_datadir = '/store/user/adas/New_storeD35/PreMixNew/SingleNuE/2020_03_14_18_26_27/'
    elif geometry==35 and premix_method=='old':
        secondary_datadir = '/store/user/adas/New_storeD35/PreMixOld/SingleNuE/2020_03_14_20_21_25/'
    elif geometry==41 and premix_method=='old':
        secondary_datadir = '/store/user/adas/New_storeD41/PreMixOld/SingleNuE/2020_06_28_05_04_06/'
    elif geometry==41 and premix_method=='new':
        secondary_datadir = '/store/user/adas/New_storeD41/PreMixNew/SingleNuE/2021_02_12_20_24_48/'
        
    generic_initial = '/hadoop' +secondary_datadir + 'SingleNuE10_cfi_GEN_SIM_DIGI_PU'
    secondary_filepath = generic_initial + str(idx_Secondary) + '.root'
    #redflag = os.path.exists(secondary_filepath)
    print("Got Secondary Dataset File as :  ",secondary_filepath)
    #assert os.path.exists(secondary_filepath)
    
    rep = {'step1.root' : gensim_file, 'step2.root':secondary_filepath}
    rep = dict((re.escape(k), v) for k, v in rep.iteritems())
    pattern = re.compile("|".join(rep.keys()))
    CMSSW_holder_dir = cmssw_holder_dir#'/afs/crc.nd.edu/user/a/adas/Orchid/'
    ...
    substitute_file_path  = CMSSW_BASE+"/src/PreMix/substitute.py"
    ...
    os.system("./step3_helper.csh "+storage_dir+" "+str(idx_primary))
...
modify_and_cmsRun_config_file(configfile, files_list, idx_primary, idx_Secondary)
